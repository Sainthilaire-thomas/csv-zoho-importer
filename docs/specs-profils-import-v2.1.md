...
>
> Quand l'utilisateur choisit un profil existant, il adopte **toute** sa configuration :
>
> * Table Zoho cible
> * Mode d'import
> * Clé de matching (si applicable)
> * Formats de transformation
> * Alias de colonnes
>
> **Pas de modification "à la volée"** : Pour un mode différent, créer un nouveau profil ou faire un import ponctuel.
...
### 12.5 Interface de sélection de la clé

Dans StepConfig, si un mode nécessitant une clé est sélectionné :

```
┌─────────────────────────────────────────────────────────────────────┐
│  ⚠️ Ce mode nécessite une clé de référence                          │
│                                                                      │
│  Colonne(s) identifiant une ligne unique :                          │
│                                                                      │
│  [✓] N° Quittance                                                    │
│  [ ] Année                                                           │
│  [ ] Mois                                                            │
│  [ ] Site                                                            │
│                                                                      │
│  💡 La combinaison des colonnes sélectionnées doit être unique      │
...
### 13.1 Les trois chemins à l'étape Profil

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        ÉTAPE PROFIL D'IMPORT                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  📁 Fichier : QUITTANCES_12_2025.xlsx (14 lignes, 22 colonnes)             │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  ✓ Profil compatible trouvé                                         │   │
│  │                                                                      │   │
│  │  "Import Quittances" - 98% compatibilité                            │   │
│  │  Table: QUITTANCES | Mode: APPEND | Utilisé 12 fois                 │   │
│  │                                                                      │   │
│  │  [👁 Voir détails]                    [✓ Utiliser ce profil]       │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ── ou ──                                                                   │
│                                                                             │
│  ┌────────────────────────┐    ┌────────────────────────┐                  │
│  │     ➕ Créer un        │    │     ⚡ Import          │                  │
│  │    nouveau profil      │    │      ponctuel          │                  │
│  │                        │    │                        │                  │
│  │  Configuration         │    │  Sans profil,          │                  │
│  │  complète + sauvegarde │    │  config manuelle       │                  │
│  └────────────────────────┘    └────────────────────────┘                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 13.2 Chemin A : Utiliser un profil existant

```
Upload fichier
     │
     ▼
Étape Profil ─── Profil trouvé ─── [Utiliser ce profil]
     │                                      │
     │                                      ▼
     │                            Vérification mode + clé
     │                                      │
     │                    ┌─────────────────┼─────────────────┐
     │                    │                 │                 │
     │                    ▼                 ▼                 ▼
     │              Mode APPEND        Mode UPDATE       Mode UPDATE
     │              ou TRUNCATE        + clé définie     + clé vide
     │                    │                 │                 │
     │                    ▼                 ▼                 ▼
     │                   OK                OK            ❌ ERREUR
     │                    │                 │         "Profil incomplet"
     ...
     │                    - lastUsedAt
     │                    - useCount++
     │                    - nouveaux alias (si colonnes similaires)
     │                    - nouveaux formats (si issues résolues)
     │
     ▼
   [FIN]
```

### 13.3 Chemin B : Créer un nouveau profil

```
Upload fichier
     │
     ▼
Étape Profil ─── [Créer un nouveau profil]
     │
     ▼
Étape Config :
  - Sélection workspace
  - Sélection table
  - Mode d'import
  - Si mode UPDATE* : sélection clé de matching (obligatoire)
     │
     ▼
Étape Validation
     │
     ▼
Étape Résolution (si issues détectées)
     │
     ▼
Étape Review + Import
     │
     ▼
Création profil avec :
  - Colonnes détectées + acceptedNames
  - Formats résolus (dayMonthOrder, etc.)
  - Mode d'import
  - Clé de matching (si applicable)
     │
     ▼
...
### 13.4 Chemin C : Import ponctuel (sans profil)

```
Upload fichier
     │
     ▼
Étape Profil ─── [Import ponctuel]
     │
     ▼
Étape Config :
  - Sélection workspace
  - Sélection table
  - Mode d'import
  - Si mode UPDATE* : sélection clé (non sauvegardée)
     │
     ▼
Étape Validation
     │
...
## 14. Aperçu du profil (composant ProfileDetails)

### 14.1 Déclenchement

Le bouton **"Voir détails"** dans StepProfile ouvre une modale avec les informations complètes du profil.

### 14.2 Interface

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Détails du profil : "Import Quittances"                           [✕]     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  📊 INFORMATIONS GÉNÉRALES                                                  │
│  ─────────────────────────────────────────────────────────────────────────  │
│  Table Zoho        : QUITTANCES                                             │
│  Workspace         : INDICATEURS GENERAUX                                   │
│  Créé le           : 02/12/2025                                             │
│  Dernière utilisation : 04/12/2025 (il y a 2 heures)                       │
│  Nombre d'imports  : 12                                                     │
│                                                                             │
│  ⚙️ CONFIGURATION D'IMPORT                                                  │
│  ─────────────────────────────────────────────────────────────────────────  │
│  Mode par défaut   : APPEND (ajouter à la fin)                              │
│  Clé de matching   : N° Quittance                                           │
│                      (ou "Non définie" si absent)                           │
│                                                                             │
│  📋 COLONNES CONFIGURÉES (22)                                               │
│  ─────────────────────────────────────────────────────────────────────────  │
│  │ Colonne Zoho    │ Type   │ Alias acceptés           │ Format            │
│  ├─────────────────┼────────┼──────────────────────────┼───────────────────│
│  │ Date début      │ Date   │ Date début, Date_debut   │ JJ/MM/AAAA → ISO  │
│  │ Date fin        │ Date   │ Date fin, DateFin        │ JJ/MM/AAAA → ISO  │
│  │ Durée           │ Durée  │ Durée, Duree, Duration   │ HH:mm → HH:mm:ss  │
│  │ Montant HT      │ Nombre │ Montant HT, MontantHT    │ Virgule → Point   │
│  │ ...             │ ...    │ ...                      │ ...               │
│                                                                             │
│  🔍 COMPATIBILITÉ AVEC LE FICHIER ACTUEL                                    │
│  ─────────────────────────────────────────────────────────────────────────  │
│  ✓ 20 colonnes : correspondance exacte                                      │
│  ~ 2 colonnes  : correspondance similaire (alias à ajouter)                 │
│  ✗ 0 colonnes  : non reconnues                                              │
│                                                                             │
│                                              [Utiliser ce profil]           │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
## 15. Flux de mise à jour des profils (implémenté)
...
Quand un fichier avec des noms de colonnes légèrement différents est importé avec un profil existant :

```
Profil initial :
  acceptedNames: ["Date début"]

Fichier importé avec colonne "Date_debut" (match "similar") :
  → Après import réussi, alias ajouté automatiquement
  
Profil mis à jour :
  acceptedNames: ["Date début", "Date_debut"]
```

### 15.3 Accumulation des formats de date

Quand une date ambiguë est résolue par l'utilisateur :

```
Profil initial :
  config.dayMonthOrder: undefined (non défini)

Utilisateur résout : "C'est JJ/MM/AAAA"
  → Après import réussi, format sauvegardé

Profil mis à jour :
  config.dayMonthOrder: "dmy"

Import suivant :
  → Issue ambiguous_date_format non créée (format connu)
  → Skip automatique de la résolution
```

### 15.4 Gestion du conflit 409

Si un utilisateur choisit "Créer un profil" alors qu'un profil existe déjà pour cette table :

1. POST /api/profiles retourne 409 avec `existingProfileId`
2. Le système fait automatiquement un PUT pour enrichir le profil existant
3. Les colonnes du nouveau fichier enrichissent les colonnes existantes
4. L'import continue normalement (comportement non bloquant)

---

## 16. Messages d'erreur

### 16.1 Profil avec mode UPDATE mais sans clé

```
┌─────────────────────────────────────────────────────────────────┐
│  ❌ Profil incomplet                                            │
│                                                                 │
│  Le profil "Import Quittances" utilise le mode UPDATEADD qui   │
│  nécessite une clé de référence, mais aucune clé n'est         │
│  configurée.                                                    │
│                                                                 │
│  Options :                                                      │
│    [Créer un nouveau profil]    [Faire un import ponctuel]     │
└─────────────────────────────────────────────────────────────────┘
```

### 16.2 Import ponctuel avec mode UPDATE sans clé

```
┌─────────────────────────────────────────────────────────────────┐
│  ⚠️ Clé de référence requise                                    │
│                                                                 │
│  Le mode "Mettre à jour ou ajouter" nécessite une colonne de   │
│  référence pour identifier les lignes existantes.              │
│                                                                 │
│  Veuillez sélectionner la colonne identifiant unique.          │
└─────────────────────────────────────────────────────────────────┘
```
